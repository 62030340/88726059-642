...
function validatePassword(passid, min, max) {
    ...
    if (passid.value == "") {
        ...
        error = "Password มีตัวอักษรที่ไม่ได้อนุญาต\n";
        passid.style.background = 'Yellow';
        alert(error);
        passid.focus();
        return false;
    } else if ((passid.value.search(/[a-zA-Z]+/) == -1) || passid.value.search(/[0-9]+/) == -1)) {
    error = "Password ต้องมีเลขอย่างน้อย 1 ตัว\n";
    passid.style.background = 'Yellow';
    alert(error);
    passid.focus();
    return false;
} else { passid.style.background = "White"; }
return true;
}
